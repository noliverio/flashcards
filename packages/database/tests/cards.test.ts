...
const { getCardByID, createNewCard, getCategoryByID, deleteCard, createNewCategory, listCategories, deleteCategory } = cardsModule
...
describe('cards queries', () => {
  ...
  it('getCardByID returns validated card when found (number and string id)', async () => {
    const mockRow = {
      ...
      use_history: { uses: [] },
      next_session: 1,
      category: { id: 2, category_name: 'cat', session_number: 0, last_play_date: '2020-01-01' }
    }
  ...
  it('createNewCard inserts validated card and returns result', async () => {
    const cardInput = { category_key: 1, question: 'q', answer: 'a', next_session: 1, use_history: { uses: [] } }
    const returning = [{ id: 42, ...cardInput }]
    mockedDb.insert.mockImplementation(() => ({ values: () => ({ returning: () => Promise.resolve(returning) }) }))
  ...
  it('getCategoryByID returns validated category when found and null when not', async () => {
    const mockCategory = { id: 2, category_name: 'cat', session_number: 0, last_play_date: '2020-01-01' }
    mockedDb.select.mockImplementation(() => ({ from: () => ({ where: () => Promise.resolve([mockCategory]) }) }))

    const result = await getCategoryByID(2)
    expect(result).toMatchObject({ id: 2, category_name: 'cat' })

    mockedDb.select.mockImplementation(() => ({ from: () => ({ where: () => Promise.resolve([]) }) }))
  ...
  it('deleteCard calls delete and returns result', async () => {
    ...
    mockedDb.delete.mockImplementation(() => ({ where: () => ({ returning: () => Promise.resolve(returning) }) }))
  ...
  it('createNewCategory inserts validated category and returns result', async () => {
    const categoryInput = { category_name: 'new-cat', session_number: 0, last_play_date: '2020-01-01' }
  ...
  it('listCategories queries categories and returns undefined (current implementation)', async () => {
    const mockList = [{ id: 1, category_name: 'cat', session_number: 0, last_play_date: '2020-01-01' }]
    mockedDb.select.mockImplementation(() => ({ from: () => Promise.resolve(mockList) }))
  ...
  it('deleteCategory deletes related cards then deletes category and returns result', async () => {
    ...
    mockedDb.delete
      ...
      .mockImplementationOnce(() => ({ where: () => ({ returning: () => Promise.resolve(returning) }) }))
